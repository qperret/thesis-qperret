...
%In the previous section, we overviewed the two main approaches for computing bounds on WCETs. Static analysis techniques provide bounds in which the confidence we can have is directly correlated to the fidelity of the processor's model on which the analysis was led. On the other hand, measurement-based techniques aim at hiding the complexity of the hardware target by deriving bounds from experiments. Still, the soundness of such bounds is function of the quality of the experiments both in terms of functional and hardware state coverage, which are difficult to obtain with modern software and/or modern processors.  %Static analysis methods have shown good results when applied on monocore sequential code but usually face difficulties to provide tight bounds in a context of preemptive multi-tasking~\cite{HDRRochange, Bourgade2012}.
...
%\subsubsection{Cache-related preemption delays}
%\subsubsection{Shared caches}

%\subsection{Shared resources of many-core processors}
...
%    \label{fig_stateOfTheArt_NoCswitch}
%\end{figure}
...
\end{equation}
Figure~\ref{fig_stateOfTheArt_NCcumulCurves} shows two cumulative function representing an input flow $x(t)$ and an output flow  $y(t)$ with a graphical representation of the backlog (\tikz{\draw[latex-latex, densely dotted, thick] (0.0,0.5) -- (0.7,0.5);}) and delay (\tikz{\draw[latex-latex, dashed, thick] (0.0,0.5) -- (0.7,0.5);})functions.
...
%We overviewed in Section~\ref{sec_stateOfTheArt_accountInterferences} the methods for computing interference penalties when accessing the NoC and the DRAM subsystem on a many-core processor. In general, the performance that can be expected from those purely analytical methods, when applied on COTS, are limited by pathological situations. Most of existing COTS have been designed to optimize average performance while worst-case performance is usually left over.
...
%Several contributions in the literature have been proposed to tackle the predictability requirements of hard-real time systems using custom-made processors, memory hierarchies, interconnects and networks.
%Bate \etal analyze in~\cite{Bate2001} the usage of modern processors in safety critical applications. They identify several benefits of COTS processors against bespoke processors such as their extensive operational experience or the availability of widely used tools. However, they also emphasize several advantages of using custom-made processors. In particular, they point out the flexibility offered by bespoke solutions, especially to enable the designers to manage the trade-off on performance versus predictability. Moreover, they argue that a white-box approach greatly simplifies the program analysis. \\

%In~\cite{Edwards2007}, Edwards and Lee highlight the strive for average-performance of modern computer architecture and the negligence of software timing predictability and repeatability. They argue that major modifications in all the abstraction layers of computing (from low level pipelining techniques, memory hierarchies and interconnects up to programming languages and task scheduling) are required to design precision timed (\emph{PRET}) machines ``\emph{whose temporal behavior is as easily controlled as their logical function}''. \\

%All the components of a many-core processor, including the cores, the NoC and the DDR-SDRAM controller, have been studied in the literature, at least separately. From these studies, several contributions have been made to ease their temporal analysis by adapting their design.
...
%Overall, the high level CompSOC philosophy of providing non-shared execution environments to applications by leveraging a property of temporal isolation is relatively close to our approach. However, the actual implementation of the key concepts behind this philosophy has fundamental differences. In the CompSOC approach, the temporal isolation on which the framework relies is assumed to be effective thanks to a hardware and software co-design. In this thesis, we try to evaluate if the temporal isolation property can be enforced by software only using an execution model on a COTS hardware. Moreover, we investigate the propagation of the constraints coming from the execution model on the automatic mapping procedures of high level applications. 
...
%The management of shared resources in multi- and many-core processors is a major obstacle to time predictability~\cite{Wilhelm2012}. For this reason, 
...
%Although bespoke processors, interconnects and IO controllers certainly provide the most elegant solution to meet the timing requirements of hard real-time systems, the utilization of COTS components is still preferred in many industrial projects. This choice is motivated by several reasons. Firstly COTS hardware components enable both design costs and time-to-market reduction. Indeed, buying an existing, already widely used, supported and supplied with tools component on the market allows to trade the time and cost incurred for the development, testing and validation of a bespoke component against additional efforts in the software design to manage the caveats of the COTS target. This trade-off has been seen as beneficial in many cases. \\

%Secondly, existing modern COTS processors seem to offer a computation power that is, at least, one order of magnitude higher than existing (industrially-used) custom-made processors. For instance, the bespoke processor from \emph{Control and Data Services} , part of the Rolls-Royce Group, does not feature any instruction or data caches~\cite[Section IV.C]{Law2016}. Although this obviously helps to improve the time-invariability of executions, this also brings several questions in terms of performances. \\

%Finally, it can be argued that the complete end-to-end design of state-of-the-art microprocessors is out of the core business for many industrial real-time systems designers such as the aircraft manufacturers.  In order to sustain the growing demands of computation power in modern embedded systems, the choice of a powerful microprocessor that does not abandon 40 years of progress in computer architecture is unavoidable. Yet, the workforce needed for the design and the investments required for the production of modern microprocessors is enormous. In this context, the usual (but not systematic) decision of aircraft manufacturers to disregard  bespoke processors in favor of COTS is understandable. Nonetheless, the academic efforts emphasizing the opportunities offered by time-predictable hardware are of main interest from an industrial point of view. The integration of such technologies into COTS components would be a significant asset to the design of future avionics computers. The challenge now is to convince chip manufacturers to do so.\\
...
%Giannopoulou \etal proposed\cite{Giannopoulou2013_EMSOFT}
...
%In~\cite{BoniolCNP12}, Boniol \etal address the problem of the automatically computing the schedule of communication and execution slices of a dependent task set on a multi-core architecture. \\
...
%In addition, Jegu \etal provide an ILP formulation of the task slicing and scheduling problem that takes into account the temporal requirements of the tasks, their inherited dependencies (precedence constraints between communication and execution slices for example) and the low level implementation constraints such as the sizes of the caches. \\
