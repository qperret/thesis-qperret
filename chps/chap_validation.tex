...
%The constraint-solving problem is inherently complex because of the high degree of freedom in the formulation. No assumption is made on the location of sub-tasks to \PN{}s or on the schedule of data on \PC{}s despite the inter-correlated nature of the problem. In order to ease the solving, we slightly decreased this complexity using two different optimisations and transformations on the application model. These modifications are detailed below.

%\subsubsection{Application model transformations}
%Our application model describes not only the internal structure of tasks using the sub-tasks but also the data exchanged between sub-tasks. As we explained in Sections~\ref{ssection_implemExecModel_systick} and~\ref{sssec_validation_PCutilConst}, the number and the size of data is of major importance for performance. 
...
%\subsubsection{Local activations and deadlines of jobs}
%\textbf{TODO}
...
By using symmetric \PC{}s, we aim at simplifying the automatic expression of budgets to perform parametric studies and we argue that prompt \PC{}s should help shorten the delays involved by precedences with forward data. The evaluation of the CSP under different experimental conditions with different budgets featuring asymmetric or slow \PC{}s appears as a good opportunity of future work.
